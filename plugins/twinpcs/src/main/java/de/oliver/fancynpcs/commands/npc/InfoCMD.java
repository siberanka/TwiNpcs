...
public enum InfoCMD {
    ...
    @Permission("twinpcs.command.npc.info")
    public void onInfo(
        ...
        translator.translate("npc_info_general")
                ...
                .replace("model_provider", npc.getData().getModelProvider().name().toLowerCase())
                .replace("model_id", npc.getData().getModelId() == null ? "vanilla" : npc.getData().getModelId())
                .replace("bedrock_type", npc.getData().getBedrockFallbackType() == null
                        ? ((SimpleMessage) translator.translate("disabled")).getMessage()
                        : npc.getData().getBedrockFallbackType().name().toLowerCase())
                .replace("bedrock_offset", npc.getData().getBedrockOffsetX() + ", "
                        + npc.getData().getBedrockOffsetY() + ", " + npc.getData().getBedrockOffsetZ())
                .replace("bedrock_interactions", getTranslatedState(npc.getData().isBedrockInteractionForwarding()))
                .replace("bedrock_skin", npc.getData().isBedrockMirrorSkin()
                        ? "mirror"
                        : npc.getData().getBedrockSkinData() == null
                                ? "default"
                                : npc.getData().getBedrockSkinData().getIdentifier())
