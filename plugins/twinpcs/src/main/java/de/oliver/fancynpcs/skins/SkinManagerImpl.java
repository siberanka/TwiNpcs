...
public class SkinManagerImpl implements SkinManager, Listener {
    ...
    public final static ScheduledExecutorService EXECUTOR = Executors.newScheduledThreadPool(
            ...
            new ThreadFactoryBuilder()
                    .setNameFormat("TwiNpcs-Skins-%d")
                    .build()
    );

    private final String SKINS_DIRECTORY = "plugins/TwiNpcs/skins/";
...
//        mineSkinQueue.add(new MineSkinQueue.SkinRequest(uuid.toString(), genReq));
    ...
    public void onSkinGenerated(SkinGeneratedEvent event) {
        ...
        for (Npc npc : FancyNpcs.getInstance().getNpcManager().getAllNpcs()) {
            ...
            if (matchesGeneratedSkin(skin, event.getId())) {
                npc.getData().setSkinData(withGeneratedTexture(skin, event.getSkin()));
                updated = true;
            }

            SkinData bedrockSkin = npc.getData().getBedrockSkinData();
            if (matchesGeneratedSkin(bedrockSkin, event.getId())) {
                npc.getData().setBedrockSkinData(withGeneratedTexture(bedrockSkin, event.getSkin()));
                updated = true;
            }
    ...
    private boolean matchesGeneratedSkin(SkinData configuredSkin, String generatedId) {
        if (configuredSkin == null) {
            return false;
        }

        String id = configuredSkin.getParsedIdentifier();
        ...
        return id.equals(generatedId);
    }

    private SkinData withGeneratedTexture(SkinData configuredSkin, SkinData generatedSkin) {
        return new SkinData(
                configuredSkin.getIdentifier(),
                generatedSkin.getVariant(),
                generatedSkin.getTextureValue(),
                generatedSkin.getTextureSignature()
        );
    }
